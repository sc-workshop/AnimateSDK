...
namespace Animate::Publisher {
    ...
    public:
        ...
        public:
            Iterator(const FilledElementPathSegment& path, bool is_end_node = false) :
                m_path(path) {
                if (!is_end_node) {
            ...
            bool operator==(Iterator& other) const { return m_step >= other.m_step; }
            bool operator!=(Iterator& other) const { return !operator==(other); }

            // Data Access
            Point2D operator*() const { return m_path.Rasterize(m_step); }
            Point2D operator->() const { return operator*(); }
    ...
    public:
        ...
        bool operator!=(const FilledElementPathSegment& other) const { return !operator==(other); };
    };
}