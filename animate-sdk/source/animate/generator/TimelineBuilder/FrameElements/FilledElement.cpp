...
namespace Animate::Publisher {
    FilledElement::FilledElement(SymbolContext& symbol, FCM::AutoPtr<DOM::FrameElement::IShape> shape, const std::optional<DOM::Utils::MATRIX2D> matrix) :
        ...
        auto stroke_generator = context.GetService<DOM::Service::Shape::IShapeService>(DOM::FLA_SHAPE_SERVICE);

        auto region_generator = context.GetService<DOM::Service::Shape::IRegionGeneratorService>(DOM::FLA_REGION_GENERATOR_SERVICE);
    ...
    bool FilledElement::operator==(const FilledElement& other) const {
        if (fill.size() != other.fill.size() || stroke.size() != other.stroke.size()) {
            return false;
        }
    ...
    DOM::Utils::RECT FilledElement::Bound() const {
        DOM::Utils::RECT result {{-std::numeric_limits<float>::max(), -std::numeric_limits<float>::max()},
