...
namespace Animate::DocType
{
	...
	public:
		static FCM::Result Register(
			FCM::PIFCMDictionary plugins, 
			FCM::ConstFCMCLSID docTypeID, 
			std::string name, std::string universal_name, std::string description = ""
		)
		{
			FCM::Result res = FCM_SUCCESS;
...

			{
...

				{
					...
					res = plugin->AddLevel((const FCM::StringRep8)kApplicationCategoryKey_DocType, category.m_Ptr);

					{
						...
						res = category->Add(
							...
							FCMDictRecTypeID::StringRep8,
							(FCM::PVoid)universal_name.c_str(),
							(FCM::U_Int32)universal_name.length() + 1
						);
						...
						res = category->Add(
							...
							FCMDictRecTypeID::StringRep8,
							(FCM::PVoid)description.c_str(),
							(FCM::U_Int32)description.length() + 1
						);
					}
				}
			}
