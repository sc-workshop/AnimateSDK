...
namespace Animate::Publisher {
    ...
    public:
        ...
        FCM::Result _FCMCALL Publish(DOM::PIFLADocument /*document*/,
    ...
    inline FCM::Result RegisterPublisher(FCM::PIFCMDictionary plugins, FCM::FCMCLSID document_id, FCM::FCMCLSID publisher_id, const ModuleInfo& module_info) {
        FCM::Result result;
...

        {
            ...
            result = plugins->AddLevel(((std::string) publisher_id).c_str(), plugin.m_Ptr);

            // Level 2 Dictionary
            FCM::AutoPtr<FCM::IFCMDictionary> category;
            result = plugin->AddLevel((const FCM::StringRep8) Application::kApplicationCategoryKey_Publisher, category.m_Ptr);

            // Level 3 Dictionary
            result = category->Add((const FCM::StringRep8) Application::kApplicationCategoryKey_Name,
                                   FCMDictRecTypeID::StringRep8,
                                   (FCM::PVoid) module_info.name,
                                   (FCM::U_Int32) std::strlen(module_info.name) + 1);

            result = category->Add((const FCM::StringRep8) Application::kApplicationCategoryKey_UniversalName,
                                   FCMDictRecTypeID::StringRep8,
                                   (FCM::PVoid) module_info.universal_name,
                                   (FCM::U_Int32) std::strlen(module_info.universal_name) + 1);

            result = category->Add((const FCM::StringRep8) Application::kApplicationPublisherKey_UI,
                                   FCMDictRecTypeID::StringRep8,
            ...
            // Level 4 Dictionary
            result = pDocs->Add(((std::string) document_id).c_str(), FCMDictRecTypeID::StringRep8, (void*) "", 1);
        }
